...
TABLES = ["classes", "equipment", "monsters", "raids", "floors", "appearances", "drops", "raid_drops", "skills", "skill_parents", "skill_ranks"]
...
def normalize(data):
    ...
    for table in TABLES:
        data[table].sort(key=lambda r: (str(r.get("id", r.get("skill_id", ""))), str(r.get("parent_id", "")), int(r.get("rank", 0))))
    ...
    todo = [ROOT / "game/scripts/game_database.gd", Path(__file__).resolve()]
    for table in TABLES:
...
def validate(data):
    ...
    indexes = {}
    for table in TABLES:
        if table in ("skill_parents", "skill_ranks"):
            continue
        rows = data[table]
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="database-", dir=ROOT / "runtime") as temporary:
        ...
        manifest = {"schema_version": 1, "counts": data["metadata"]["counts"], "files": {p.name: {"bytes": p.stat().st_size, "sha256": hashlib.sha256(p.read_bytes()).hexdigest()} for p in files}}
