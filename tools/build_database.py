...
TABLES += exploration_db_rules.TABLES
...
def normalize(data):
    ...
    data = exploration_db_rules.enrich(data, ROOT)
    ...
    todo = [ROOT / "game/scripts/game_database.gd", Path(__file__).resolve(), Path(art_registry.__file__).resolve(), Path(game_db_rules.__file__).resolve(), Path(exploration_db_rules.__file__).resolve()]
    todo += [ROOT / path for path in game_db_rules.SOURCES.values()]
    todo += [ROOT / path for path in exploration_db_rules.SOURCES.values()]
...
def validate(data):
    ...
    exploration_db_rules.validate(data)
    art_registry.validate(data, ROOT)
    assert len(data["classes"]) == 20
    assert len(data["equipment"]) == 2500
    assert len(data["monsters"]) == 24+len(json.loads((ROOT/"game/data/monster_ecology_v071.json").read_text("utf8"))["monsters"])
...
SCHEMA += exploration_db_rules.SCHEMA
...
def build_sqlite(path, data):
    ...
    exploration_db_rules.insert(con, data)
