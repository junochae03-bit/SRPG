...
RUN=ROOT/'runtime/v05-checks'/time.strftime('%Y%m%d-%H%M%S');RUN.mkdir(parents=True)
...
def run(test,args=None,graphics=False):
    ...
    markers=[line for line in output.splitlines() if any(t in line for t in ['TESTS checks=','LEGACY_SAVE_PASS','FOREST_STABILITY','VISUAL_V05_PASS'])]
...
for test in ['forest_stability','rules','inventory_grid','inventory_ui','combat','skills_v04','loot_v04','single_player','expansion_ui','appearance_v041','expansion_v05','polish_v05','monsters_v05']:run(test)
...
(ROOT/'artifacts/v05_verification.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),'utf8')
print('V05_GATE PASS checks='+str(checks),flush=True)
