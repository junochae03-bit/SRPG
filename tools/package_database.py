...
with zipfile.ZipFile(archive) as package:
    assert len(package.namelist())==6 and package.testzip() is None
digest=hashlib.sha256(archive.read_bytes()).hexdigest()
(out/(prefix+'-SHA256.txt')).write_text(digest+'  '+archive.name+'\n','utf8')
report={'status':'PASS','version':VERSION,'filename':archive.name,'bytes':archive.stat().st_size,'sha256':digest,'entries':6,'counts':manifest['counts']}
