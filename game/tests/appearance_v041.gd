...
func run():
	...
	check(Content.AVATARS.size()==37,"all 37 supplied characters available")
	...
	check(game.bag.avatar_picker.item_count==38,"base selector contains automatic plus 37 options")
	for i in range(Content.AVATARS.size()):
		game.bag.avatar_picker.item_selected.emit(i+1)
		var key=Content.AVATARS.keys()[i]
		check(p.avatar==key and p.costume=="none","base selector applies "+key)
		check(game.bag.portrait.texture==GatArt.texture(key,0) and game.hud.portrait==GatArt.portrait(p),"base previews agree "+key)
	...
	for key in Content.COSTUMES:
		game.bag.costume_picker.item_selected.emit(Content.COSTUMES.keys().find(key))
		check(p.costume==key and p.avatar==avatar,"costume does not erase base selection "+key)
		if key!="none" and not Content.gat_appearance(p):check(game.bag.portrait.texture==game.textures[Content.costume_role(p)].idle[0],"Stellive costume uses original sprite "+key)
	session.act("costume","none");session.save_game();var saved=session.parse_save(session.save_path())
	check(saved.avatar==avatar and saved.costume=="none" and saved.schema_version==5,"base selection persisted in v5")
