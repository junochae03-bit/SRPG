...
func run():
	...
	for cls in Creation.CLASSES:
		...
		check(p.defense==sheet.stats.fortitude*2+6 and local.sim.damage_for(p,"magic")==22+sheet.stats.power*2,"created stats and starter gear change actual combat")
	...
	for bad_value in [-1,1.5,"3",INF]:
		var bad=good.duplicate(true);bad.stats.power=bad_value;check(not Creation.reason(bad).is_empty(),"invalid stat rejected")
	...
	var bad=good.duplicate(true);bad.stats.power+=1;check(not Creation.reason(bad).is_empty(),"overspend rejects")
	bad=good.duplicate(true);bad.stats.power-=1;check(not Creation.reason(bad).is_empty(),"unspent creator points reject")
	...
	var p=local.sim.players[1];p.level=20;p.gold=817;p.stats.power=11;p.skill_ranks={"blade":2};p.tutorial_done=true;local.save_game()
	var old=local.sim.persistent(1);old.world_seed=local.world_seed;old.schema_version=6;old.erase("stat_schema_version");old.stats={"strength":11};old.creation_points=10;old.constellation_allocations={"forged":1};write_json(local.save_path(),old)
	...
	if reloaded!=stable:
		for index in range(mini(reloaded.length(),stable.length())):
			if reloaded[index]!=stable[index]:print("SAVE_BYTES ",stable.substr(maxi(0,index-40),120)," -> ",reloaded.substr(maxi(0,index-40),120));break
		var before_data=JSON.parse_string(stable);var after_data=JSON.parse_string(reloaded)
		for field in before_data:
			if before_data[field]!=after_data.get(field):print("SAVE_DIFFERENCE ",field," ",before_data[field]," -> ",after_data.get(field))
	check(reloaded==stable,"second load does not refund or alter property")
