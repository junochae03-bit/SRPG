...
func tutorial_arrival():
	for cls in ["warrior","ranger","mage"]:
		...
		check(p.hp==p.max_hp and p.max_hp==148+int(stats.get("vitality",0))*8 and p.stamina==3,cls+" normal level-up heals with vitality and without replenishing stamina")
		...
		check(session.act("bind_skill","skill_q:"+cls+"_fan") and session.sim.action(1,"stat","power"),cls+" normal town binding and stat investment consume earned budget")
