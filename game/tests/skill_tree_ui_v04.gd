...
func run():
	...
	tree.refund_mode=false;tree.search.text="회피의 탄력";tree.apply_search();check("회피" in tree.selected.tooltip_text or "탄력" in tree.selected.tooltip_text,"Korean synergy search retains full matched name")
	tree.show_branch(0);tree.search.text="압축 파쇄";tree.apply_search();check(tree.graph.scope_cluster==4 and tree.nodes[tree.choice].visible,"cross-branch search reveals matching choice")
