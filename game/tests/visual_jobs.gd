...
func run():
	...
	game.session.save_directory="/tmp/srpg-visual-jobs-"+str(Time.get_ticks_usec());game.join_game();game.session.set_physics_process(false);game.set_physics_process(false)
	...
	for cls in ["breaker","infighter","martialist","thief","reaper","gambler","summoner"]:
