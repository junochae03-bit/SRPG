...
func identity_except_costume(p:Dictionary,sim)->Dictionary:
	var result={}
	for field in ["class_id","avatar","stats","creation_points","inventory","equipment","equipped","bag_positions","materials","gold","level","xp","skill_ranks","constellation_allocations","skill_loadout","training_given"]:result[field]=p.get(field)
...
func run():
	...
	var first_page_custom=panel.avatar_keys.slice(0,10).filter(func(id):return Costumes.recognizes(id))
	check(Costumes._sheet_textures.size()<=first_page_custom.size(),"only visible first-page custom portraits load")
	panel.name_field.text="별빛코스튬모험가";panel.name_field.text_changed.emit(panel.name_field.text)
	for cls in Creation.CLASSES:
		click(panel.job_buttons[cls]);await process_frame
		...
		for id in ids:
			...
			check(Creation.reason(sample).is_empty()==(id in Content.costume_options(cls)),cls+" creator enforces appearance matching "+id)
	var registered=[];var creator_sheets=[]
	for index in player_ids.size():
		var id=player_ids[index];var cls=allowed_class[id];click(panel.job_buttons[cls]);panel.sheet.stats=Creation.suggested(cls);panel.refresh();await process_frame
		var at=panel.avatar_keys.find(id);var target_page=int(at/10)
		check(at>=0 and Content.COSTUMES[id]==Costumes.catalog()[id].name,id+" named creator and inventory registration")
		while panel.avatar_page<target_page:click(panel.next_button);await process_frame
		while panel.avatar_page>target_page:click(panel.previous_button);await process_frame
		var b=panel.avatar_buttons[at%10];var picture=b.find_children("*","TextureRect",true,false).filter(func(child):return child.texture==Gat.portrait({"costume":id}))
		check(picture.size()==1,id+" visible page thumbnail consumes custom portrait")
		check(panel.avatar_grid.get_global_rect().encloses(b.get_global_rect()),id+" thumbnail contained in page grid")
		click(b);await process_frame
		check(panel.sheet.costume==id and panel.sheet.avatar=="auto",id+" actual thumbnail selects costume independently of avatar")
		check(Creation.reason(panel.sheet).is_empty() and not panel.create_button.disabled,id+" selected sheet validates")
		check(panel.portrait.sheet.costume==id and Gat.frame(panel.portrait.sheet,0.).costume_id==id,id+" creator preview consumes selected costume")
		label_fits(panel.appearance_label,id+" creator name");label_fits(panel.page_label,id+" creator page")
		var frame=Gat.frame(panel.sheet,0.);var drawn=panel.portrait.draw_rect_for(frame);var factor=drawn.size.x/frame.texture.get_width()
		var opaque=frame.texture.get_image().get_used_rect();var draw_bounds=Rect2(panel.portrait.global_position+drawn.position+Vector2(opaque.position)*factor,Vector2(opaque.size)*factor)
		var safe=Rect2(panel.global_position+Vector2(38,panel.appearance_label.get_rect().end.y+6),Vector2(424,panel.avatar_grid.position.y-panel.appearance_label.get_rect().end.y-10));check(safe.encloses(draw_bounds),id+" creator sprite does not cover labels or thumbnail grid")
		registered.append(id);creator_sheets.append(panel.sheet.duplicate(true))
		if index in [0,15,29]:await capture("creator-"+id)
	check(registered.size()==30 and panel.avatar_keys.count(player_ids[-1])==1,"all thirty allowed costumes reached through class-filtered actual buttons")
	check(panel.next_button.disabled,"last creator page cannot advance beyond list")
	...
	check(game.bag.costume_picker.item_count==Content.costume_options(p.class_id).size(),"inventory lists current class appearances only")
	for index in player_ids.size():
		...
		check(selected>=0 and game.bag.costume_keys==Content.costume_options(cls),id+" inventory exposes only compatible appearances")
		...
		check(game.hud.portrait==Costumes.portrait(p),id+" HUD consumes selected cropped portrait")
	...
	# Independently create and reload every actually selected creator sheet.
	for index in player_ids.size():
		var id=player_ids[index];var sheet=creator_sheets[index];var session=Local.new();root.add_child(session);session.set_physics_process(false);session.save_directory=folder.path_join(id)
		...
		check(parsed!=null and parsed.costume==id and parsed.avatar=="auto" and parsed.stats==sheet.stats,id+" parser accepts saved costume and creation data")
	...
	var invalid=creator_sheets[0].duplicate(true);invalid.costume="missing-costume";check(not Creation.reason(invalid).is_empty() and Creation.player_data(invalid).is_empty(),"invalid creator costume never produces save data")
