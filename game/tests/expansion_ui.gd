...
func run():
	...
	check(game.skill_tree.nodes.size()==50,"class has fifty visible nodes")
	...
	check(saved.costume==Content.COSTUMES.keys().back() and saved.class_id=="ranger","costume and class persist")
