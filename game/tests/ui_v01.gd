...
func run():
	...
	for facility in ["shop","smith","alchemy","guild","inn"]:
		...
		check(panel.visible and panel.title.text==World.FACILITIES[facility].name and panel.facility_icon.texture!=null and panel.body.position.x==26 and panel.body.size.x==1258,"facility identity and full transaction workspace "+facility)
