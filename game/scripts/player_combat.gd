...
func hit(p:Dictionary,e:Dictionary,amount:int,source:Variant=null,attribution:Variant=null)->bool:
	...
	p.combat_time=4.0
	if not preload("res://scripts/enemy_defense.gd").type_for(e).is_empty():amount=maxi(1,roundi(amount*preload("res://scripts/enemy_defense.gd").factor(sim,e,hit_context)))
