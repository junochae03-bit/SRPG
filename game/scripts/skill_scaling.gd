...
static func profile(node:Dictionary,rank:int,bonuses:Dictionary={})->Dictionary:
	...
	var cooldown_reference=float(result.cooldown)
	result=preload("res://scripts/constellation_effects.gd").resolve(bonuses.get("player",{}),node,result)
	result=preload("res://scripts/stat_specialization.gd").apply_base(bonuses.get("player",{}),node,result)
	return preload("res://scripts/equipment_special_stats.gd").profile(bonuses.get("player",{}),node,result,cooldown_reference)
