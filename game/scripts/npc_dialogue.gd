...
func open(key:String):
	...
	if key=="church":service_button.text="쇠약을 치료한다"
	story_button.disabled=false;game.session.paused=true;game.toast.hide();show()
func tell_story():
	if facility=="church":dialogue.text="쇠약은 이곳에서 치료해 드려요. 최대 생명력이 줄어든 부상은 여관에서 쉬면 낫습니다.";story_button.disabled=true;return
