...
const SKILLS = preload("res://scripts/skill_catalog.gd").NODES
...
static func can_invest(player: Dictionary, node_id: String) -> bool:
	...
	for node in SKILLS.get(player.get("class_id","warrior"),SKILLS.warrior):
		...
		if int(player.skill_ranks.get(node_id,0))>=3:return false
		if node.parents.is_empty():return true
		for parent in node.parents:
			if int(player.skill_ranks.get(parent,0))>=1:return true
		return false
	return false
