...
static var MATERIALS = preload("res://scripts/exploration_crafting_data.gd").names()
static var CLASSES = {
...
static func avatar_options(class_id:String)->Array:
	return preload("res://scripts/appearance_rules.gd").avatars(base_class(class_id))
static func costume_options(class_id:String)->Array:
	initialize_jobs();var result=[];var family=base_class(class_id)
	for key in COSTUMES:
		if preload("res://scripts/appearance_rules.gd").allows_costume(key,class_id,family):result.append(key)
	return result
static func appearance_allowed(class_id:String,avatar:String,costume:String)->bool:
	return avatar in avatar_options(class_id) and costume in costume_options(class_id)
static func normalize_appearance(p:Dictionary):
	var class_id=str(p.get("class_id","warrior"))
	if p.get("avatar","auto") not in avatar_options(class_id):p["avatar"]="auto"
	if p.get("costume","none") not in costume_options(class_id):p["costume"]="none"
...
static func icon_texture(item:Dictionary)->Texture2D:
	if item.get("category","") in ["weapon","armor","accessory"]:return preload("res://scripts/equipment_art.gd").texture(item)
	...
	match item.get("category","weapon"):
		...
		"material":
			key=str(item.get("material",str(item.get("id","")).trim_prefix("@mat:")))
			if preload("res://scripts/exploration_crafting_data.gd").materials().has(key):return preload("res://scripts/icon_library.gd").texture(preload("res://scripts/exploration_crafting_data.gd").icon(key))
...
static func available_points(player: Dictionary) -> int:
	initialize_jobs()
	return Build.available_points(player)

static func can_invest(player: Dictionary, node_id: String) -> bool:
	initialize_jobs()
	return Build.node_state(player,node_id).can_invest
