...
기반 N1/N2/N3: 최대 6인 방장 권위·직접 주소 연결·개인 메뉴/인벤토리·저장 ACK·공유 층 이동은 로컬 검증했다. 원정 재접속·분리 귀환·지연/손실/외부 장시간·직업 조합 밸런스가 남아 있다. 현재 변경은 협동 프로토콜 10으로 구분한다. 이전 규칙의 클라이언트는 버전 검사에서 참가를 거절한다. 자동 매칭·아군 AI 동료·턴제·미선택 항목은 추가하지 않는다.
...
| G02 전리품으로 마을 기능 해금 | 구현·로컬 검증 | town_research.gd·town_research_panel.gd / 개인 재료 투자·6종 실제 제작 해금·개인 저장; [상세](TOWN_RESEARCH.ko.md) | 연구 재료 목표(G01)·시설 외형/대사(G09) 연결 및 장기 경제 검증 |
