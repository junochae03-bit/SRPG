...
V0.5는 싱글 플레이로 개발하며 온라인 멀티플레이는 이후 단계다. 이 DB는 게임의 실제 장비·몬스터·던전·드랍·스킬·별자리 카탈로그와 완료된 원화의 파일·영역·사용 관계를 조회하는 읽기 전용 자료다. 세이브나 운영 서버 DB가 아니다. 관리 DB 스키마 5와 게임 저장 형식 v7은 서로 독립적이다.

스키마 5는 `build_nodes`에 `node_kind`·`effect_scope`·`target_active_id`·`concept_id`를 추가하고 `build_concepts`에 직업별 다섯 컨셉을 저장한다. 원기술/별자리 구분과 별개로 액티브 224·액티브 모듈 220·캐릭터 패시브 289·스탯 패시브 777개를 관리한다. 조정 수치와 전후 실행 결과는 [캐릭터 밸런스 기록](CHARACTER_BALANCE.ko.md)에 있다.
