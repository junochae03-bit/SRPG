...
`game/assets/jobs/hunter-wolf-v054.png`는 2026-09-11 내장 image_gen으로 제작한 회색 늑대 4프레임이다. 기존 동료 시트의 화풍을 참고해 성체 늑대의 대기·걷기·공격 자세를 제작했다. 투명 요청 시 배경 무늬가 남은 시안은 미사용이며, 최종 원본은 단색 자홍색 배경을 기존 크로마 셰이더로 제거한다. 이미지 원본은 보존하고 프레임별 몸체 경계·발 위치만 직업 아트 카탈로그에 기록한다. 사냥꾼 늑대는 몸체 높이 112px, 소환사의 기존 네 소환수는 224px 기준으로 표시한다. 관리 DB 동료 프레임 ID를 유지하며 실제 소비 원화·해시·크기 메타데이터를 갱신한다.
