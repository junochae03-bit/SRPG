...
4. [active] Deliver only the verified executable, PCK and required notices to D:/SRPG; keep local save files out of distribution archives. Confirm file hashes and actual launch from this exact directory. Record measured improvements and any remaining limitation. Publish the verified fix through the already authorized repository workflow without retagging V0.5.
...
Full source gate: 264,711 PASS; exported EXE: 71 runs PASS; exact D:/SRPG EXE restored two isolated record copies and 23 persistent fields each. Nine delivered files verified. Publication is the remaining step.
