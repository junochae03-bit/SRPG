...
[Windows 빌드·DB](https://github.com/junochae03-bit/SRPG/releases/tag/V0.5.2)의 게임 ZIP을 모두 풀고 **StelRPG.exe**를 실행합니다. **StelRPG.pck**를 같은 폴더에 둡니다. Godot 설치는 필요 없습니다.

빈 기록의 **모험 시작**에서 검사·궁수·마법사·도적·격투가 중 직업, 이름, 초기 능력치 **10점**을 정합니다. 외형은 직업 기본 모습으로 고정되며 **무기·머리·상의·신발 4부위가 자동 장착**됩니다. 다른 기본 외형은 500G, 코스튬은 1,200G로 마을의 **별실 부티크**에서 구매한 뒤 착용합니다. 직업에 맞는 외형만 표시되며 표시 이름은 1~24자로 바꿀 수 있습니다. 기존 기록 칸에는 레벨과 닉네임이 표시되고 **이어서 하기**로 불러옵니다. [짧은 플레이 안내](docs/PLAY_CURRENT.ko.md)
...
- ESC는 소리·화면·전투 표시 설정을 엽니다. 키 변경은 하위 화면이며 적용하거나 취소하면 설정으로 돌아옵니다.
- 스킬 가지는 전투 효과를 탭에 표시하고, 액티브·강화·핵심을 다른 모양으로 구분합니다. 현재→다음 수치와 원클릭 선행 이동으로 투자 결과를 확인합니다.
- 몬스터의 몸통 클릭이 발 위치를 향하도록 조준을 수정하고 피격 반경을 넓혔습니다. 조준한 적은 금색 발밑 고리로 표시합니다. 광역기 반경과 실제 이펙트 윤곽을 함께 넓혔습니다.
- 던전은 여섯 구조와 5칸 통로, 서로 다른 전투 무리 배치를 사용합니다. 기존 100층·10층 단위 레이드 규칙은 유지합니다.
- 마을의 작업 선택·비용·완성 결과를 재배치했습니다. 장비 분해, 물약 묶음 구매·조제, 광석 정제, 길드 납품, 여관 원정 재정비를 추가했습니다.
- 독립 코스튬 상점과 거대 허수아비 수련장을 추가했습니다. 훈련 기록·기술 대기시간 초기화를 지원하며 전리품이나 경험치는 지급하지 않습니다.
- 가방은 120칸으로 확장하고 현재 장비와 교체 후 수치를 비교합니다. 이동기 사용 후 중앙 대기시간, 우측 강공격 충전 게이지, 굵은 빨간 치명타를 표시합니다.
- 배경 소품은 생태계에 맞는 68개를 선별해 사용합니다. 저장 실패는 진행을 보존하고 재시도하며, 게임 문구의 한글 깨짐과 중국어 혼입을 자동 검사합니다.

[변경 내역](docs/RELEASE_V052.ko.md) · [전투 범위 설계](docs/COMBAT_REACH_V052.ko.md)

[스킬 선택](docs/screenshots/skills-v052.png) · [장비 비교](docs/screenshots/inventory-v052.png) · [상점 거래](docs/screenshots/shop-v052.png) · [수련장](docs/screenshots/training-v052.png)

## V0.5.1에서 이어진 개선

코스튬·장비 41개 시트의 배경색 제거를 사전에 처리해 첫 표시 때의 멈춤을 줄였습니다. 원본 그림과 준비된 RGBA의 픽셀을 비교하고, UI 스킬 정보와 배경 도형·미니맵의 반복 계산도 캐시합니다. 배경 장식은 약 45% 줄였으며 몬스터의 피격 반경을 늘렸습니다. 적의 이동 충돌이나 공격 사거리는 바꾸지 않습니다.

종이 질감은 유지하고 큰 금색 모서리를 얇게 줄였습니다. 선택 목록의 여백, 긴 이름의 줄바꿈, 초상화의 모자·머리, 긴 단축키 표시를 보완했습니다. **ESC → 키 변경**에서 동작을 선택해 키를 누르거나 키보드 위로 끌어 놓고 **적용**하면 저장됩니다. 겹치는 키는 두 동작이 서로 교환되며 **ESC**는 닫기로 고정됩니다.

[키 설정 화면](docs/screenshots/keyboard-v051.png) · [읽기 쉬운 스킬 목록](docs/screenshots/skill-list-v051.png) · [성능 측정](docs/PERFORMANCE_V051.ko.md)
...
![동화책 숲과 모험 기록 선택](docs/screenshots/title-v051.png)
![직업 기본 외형·이름·초기 능력치](docs/screenshots/character-v051.png)
...
![선행 경로·기회비용·회수 미리보기를 제공하는 별자리 지도](docs/screenshots/skills-v051.png)
![NPC 초상과 대화에서 서비스로 연결](docs/screenshots/dialogue-v051.png)
...
- 장비는 일반·레어·유니크·에픽·레전드리 테두리로 구분합니다. 무기는 현재 직업 전용, 방어구·장신구는 1차 계열 공용입니다. 가방 120칸, 장비 7부위이며 모든 아이템은 한 칸을 차지합니다.
...
표는 기본 키 배치이며 ESC에서 변경할 수 있습니다. 도감·가방·성장·대화·시설·메뉴에서는 전투가 멈춥니다. 기본 공격·강공격·회피 입력은 HUD 버튼 없이 유지됩니다. 공통 콤보 배율은 사용하지 않으며 무도가·인파이터의 고유 자원은 유지됩니다.

Windows 빌드는 실행 파일 옆 `saves`, 소스는 `runtime/saves`에 3개 슬롯을 저장합니다. 게임을 종료하고 기존 슬롯 JSON과 필요하면 `.bak`을 새 빌드의 `saves`에 복사하세요. `sprite-names.json`은 외형 별칭, `keybindings.json`은 키 배치입니다. V0.5까지의 기록은 현재 착용 외형을 보유 목록에 무료로 이관합니다. V0.3의 **저장 형식 v6**은 이름·재산·능력치·스킬·층 진행을 보존해 **v7**로 이행합니다. 원기술을 강제로 환급하지 않으며 기존 캐릭터의 생성 보너스는 0점입니다. 새 캐릭터에만 초기 10점을 제공합니다.
...
Python 3.11 이상과 Godot 4.6을 준비하고 `GODOT_EXE`를 엔진 실행 파일 경로로 지정합니다. 준비 이미지 재생성에는 Pillow와 NumPy가 필요합니다. 저장소 루트에서 실행하세요.
...
python tools/build_v01.py --version V0.5.2
python tools/test_export_v01.py --version V0.5.2
python tools/package_v01.py --version V0.5.2
python tools/package_database.py --version V0.5.2
```

[V0.5.1 검증 기록](docs/VERIFICATION_V051.json)에 전체 264,711개 검사와 실제 Windows 실행 파일 71개 실행, 최소 설치 경로의 저장 복원 결과를 기록했습니다. [성능 측정](docs/PERFORMANCE_V051.ko.md)은 CPU 처리 구간과 첫 화면 지연을 구분합니다. DB는 원본 카탈로그와 전투 소스 fingerprint를 검사해 오래된 스냅샷을 검출합니다. 게임 패키지와 개발용 DB 패키지는 별도 배포합니다.
