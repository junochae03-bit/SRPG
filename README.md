...
[Windows 빌드·DB](https://github.com/junochae03-bit/SRPG/releases/tag/V0.5.1)의 게임 ZIP을 모두 풀고 **StelRPG.exe**를 실행합니다. **StelRPG.pck**를 같은 폴더에 둡니다. Godot 설치는 필요 없습니다.

빈 기록의 **모험 시작**에서 검사·궁수·마법사·도적·격투가 중 직업, 이름, 초기 능력치 **10점**을 정합니다. 외형은 직업 기본 모습으로 고정되며 **무기·머리·상의·신발 4부위가 자동 장착**됩니다. 다른 기본 외형은 500G, 코스튬은 1,200G로 마을 상인의 **코스튬** 탭에서 구매한 뒤 착용합니다. 직업에 맞는 외형만 표시되며 표시 이름은 1~24자로 바꿀 수 있습니다. 기존 기록 칸에는 레벨과 닉네임이 표시되고 **이어서 하기**로 불러옵니다. [짧은 플레이 안내](docs/PLAY_CURRENT.ko.md)
...
종이 질감은 유지하고 큰 금색 모서리를 얇게 줄였습니다. 선택 목록의 여백, 긴 이름의 줄바꿈, 초상화의 모자·머리, 긴 단축키 표시를 보완했습니다. **ESC**는 실제 키보드 배치 화면을 엽니다. 동작을 선택해 키를 누르거나 키보드 위로 끌어 놓고 **적용**하면 저장됩니다. 겹치는 키는 두 동작이 서로 교환되며 **ESC**는 닫기로 고정됩니다.
...
python tools/build_v01.py --version V0.5.1
python tools/test_export_v01.py --version V0.5.1
python tools/package_v01.py --version V0.5.1
python tools/package_database.py --version V0.5.1
```
